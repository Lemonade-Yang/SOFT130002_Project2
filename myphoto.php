...
<script>
    ...
    function search(data,page=1) {
        $.ajax({
            ...
            data: "UID=<?=$_SESSION['UID']?>&page="+page,
            success: function (result) {
                if (parseInt(jQuery.parseJSON(result)['pageSum']) === 0) {
                    $('#picture-container').html("您还没有上传照片，赶紧点击个人中心的上传按钮增加一张吧！");
    ...
    function output(data,result){
        ...
        for(let index=0;index<num;index++){
            holder=holder+'<article>\n' +
                ...
                '                </a>\n' +
                '                <div class="action">\n' +
                '                    <button class="delete" value="delete" onclick="delete_photo('+result[index]['ImageID']+')">删除</button>\n' +
                '                    <button class="edit" value="edit" onclick="window.location.href=\'upload.php?imageid='+result[index]['ImageID']+'\'">编辑</button>\n' +
                '                </div>\n' +
                '            </article>';
        }
